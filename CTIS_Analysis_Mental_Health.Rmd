...
data_CTIS_policy_large %>% 
  filter(stay_home_requirements %in% 3) %>% select(data.country) %>%  unique() # Only turkey and india have level 3 stay home requirements
...
corrplot(cor(numeric_data, use = "complete.obs"), type="upper", order="hclust", 
         p.mat = s, sig.level = 0.01, insig = "blank")
```
