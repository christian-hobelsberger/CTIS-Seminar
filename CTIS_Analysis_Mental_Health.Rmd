...
ggplot(cor_data) + geom_col(aes(x = data.country, y = cor))


```

```{r Correlation Plots with global map}
## Correlation Plot
# new dataset with correlation between anxious and depressed
data_cor_plot <- data_CTIS_map %>% drop_na() %>% group_by(data.country, data.iso_code) %>% summarise(cor_anx_dep = cor(anxious_7d, depressed_7d))
data_cor_plot <- data_cor_plot %>% drop_na()
# join data
joinData <- joinCountryData2Map(data_cor_plot,
            nameJoinColumn = "data.iso_code")
# map plot
mapCountryData(joinData, nameColumnToPlot = "cor_anx_dep", catMethod = "FixedWidth", colourPalette = brewer.pal(7,"OrRd"))
```

```{r Correlation Plots with global map}
## Correlation Plot

# new dataset with correlation between finance and food security
data_cor_fin_food <- data_CTIS_map %>% drop_na() %>% group_by(data.country, data.iso_code) %>% summarise(cor_fin_food = cor(finance, food_security))
data_cor_fin_food <- data_cor_fin_food %>% drop_na()
# join data
joinData2 <- joinCountryData2Map(data_cor_fin_food,
            nameJoinColumn = "data.iso_code")
# map plot
mapCountryData(joinData2, nameColumnToPlot = "cor_fin_food", catMethod = "FixedWidth", colourPalette = brewer.pal(7,"OrRd"))
```


## Code from Max -> using it for correlation interactive plots (Elisa) ## 

# plot_data_map
```{r}

plot_global_map <- function(data, variable, mode = "plot", ...){
  mapdata <- data_cor_fin_food %>% 
  dplyr::select(variable, geometry, data.country, data.iso_code) 
# If we filter for specific data we lose geometries for countries with no obs at that data therefore   we need to add the geo data + NA for survey data
  missing_countries <- setdiff(world$data.country, mapdata$data.country)
  missing_data <- world %>% filter(data.country %in% missing_countries)
  missing_columns <- setdiff(colnames(mapdata), colnames(missing_data))
...
```{r}
plot_global_map(data_cor_fin_food, variable = "cor_fin_food", mode = "view", id = "data.country", popup.vars = c("Correlation between finance and food security:" = "cor_fin_food"))

plot_global_map(data_cor_fin_food, variable = "cor_fin_food", mode = "plot", 
                title ='Finance and food security correlation')+
   tm_layout(legend.position = c("left", "bottom"), 
            main.title= 'Correlation between finance and food security',
            legend.outside = TRUE,
            main.title.position = c('center', 'top'))


plot_continent_map(data_cor_fin_food, variable = "cor_fin_food",
                   area = "Europe", mode = "view", id = "data.country", 
                   popup.vars = c("Correlation between finance and food security:" = "cor_fin_food"))
```
