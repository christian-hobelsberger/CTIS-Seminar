...
week_cor <- data_CTIS_policy %>% group_by(week, continent) %>% summarise(correlation = cor(anxious_7d, depressed_7d))
# plot
ggplot(week_cor, aes(x = week, y = correlation))+ facet_wrap(~ continent, ncol = 2) + geom_line() +
  theme(axis.text = element_text(size = 8))
ggsave("weekly_correlation_anxious_dep_continent.png", width = 12, height = 9)
```
...
res <- data_CTIS %>% select(anxious_7d, depressed_7d, worried_become_ill, 
                            finance, food_security) %>% drop_na()
ggpairs(res) 
ggsave("correlation_policy_data.png", width = 12, height = 9)
