...
data_CTIS_new %>% drop_na() %>% mutate(month_new = as.Date(data.survey_date, 
...
ukraine <- Ukraine_data %>% select(-c(1:4), -10, -11, -12)
summary(ukraine)
...
## correlation week for all countries
highchart() %>% 
  hc_add_series(corr_by_month$COR, type = "line")

corr_by_month <- data_CTIS %>% drop_na() %>% group_by(month_new) %>%
  summarize(COR = cor(depressed_7d,anxious_7d)) 
