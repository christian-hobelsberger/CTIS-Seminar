...
```{r}
CTIS_micro <- readRDS("data/protected_data/CTIS_microdata_cleanV5.RDS")
```
...
model_1_downsamp <- glm(depressed ~ E3 + E4 + E2 + V11 + E8 + E5 + E7a + D7a, family=binomial(link='logit'), data= train_downsamp)
...
fitted.results_model1 <- predict(model_1, newdata= test[c("E3", "E4", "E2", "V11", "E8", "worried", "D7a", "depressed")], type='response')
fitted.results_model1_downsamp <- predict(model_1_downsamp, newdata= test[c("E3", "E4", "E2", "V11", "E8", "worried", "D7a", "depressed")], type='response')
...
```{r}
library(pROC)
test_prob <-  predict(model_1_downsamp, newdata = test[c("E3", "E4", "E2", "E5", "E7a", "D7a", "V11", "E8", "worried", "D7a", "depressed")], type = "response")
test_roc <- roc(test$depressed ~ test_prob, plot = TRUE, print.auc = TRUE)
```
```{r}
...
conf_table <- table(predicted = as.logical(fitted.results_model1), 
                    actual = test$worried)
confus_matrix <- caret::confusionMatrix(conf_table)#Huge probelem with specificity
confus_matrix
sensitivity(confus_matrix)
```
...
```
```{r}
test_pred_10 <- get_logistic_pred(model_1, test[c("E3", "E4", "E2", "V11", "E8", "worried", "D7a")], cut = 0.1)
test_pred_20 <- get_logistic_pred(model_1, test[c("E3", "E4", "E2", "V11", "E8", "worried", "D7a")], cut = 0.2)
test_pred_30 <- get_logistic_pred(model_1, test[c("E3", "E4", "E2", "V11", "E8", "worried", "D7a")], cut = 0.3)
test_pred_40 <- get_logistic_pred(model_1, test[c("E3", "E4", "E2", "V11", "E8", "worried", "D7a")], cut = 0.55)
```
```{r}
test_tab_10 <- table(predicted = as.logical(test_pred_10), actual = test$worried)
test_tab_20 <- table(predicted = as.logical(test_pred_20), actual = test$worried)
test_tab_30 <-  table(predicted = as.logical(test_pred_30), actual = test$worried)
test_tab_40 <-  table(predicted = as.logical(test_pred_40), actual = test$worried)
...
rownames(metrics) = c("c = 0.20", "c = 0.30", "c = 0.6")
metrics
```
