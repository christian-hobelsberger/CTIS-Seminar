...
model_1_downsamp <- glm(depressed ~ E3 + E4 + E2 + V11 + E8 + E5 + E7a + D7a + country_agg, family=binomial(link='logit'), data= train_downsamp)
...
fitted.results_model1 <- predict(model_1, newdata= test[c("E3", "E4", "E2", "E5", "E7a", "D7a", "V11", "E8", "worried", "D7a", "country_agg", "depressed")], type='response')
...
```{r}
library(pROC)
test_prob <-  predict(model_1_downsamp, newdata = test[c("E3", "E4", "E2", "E5", "E7a", "D7a", "V11", "E8", "worried", "D7a", "country_agg", "depressed")], type = "response")
test_roc <- roc(test$depressed ~ test_prob, plot = TRUE, print.auc = TRUE)
```
```{r}
fitted.results_model1 <- ifelse(fitted.results_model1 > 0.4,1,0)
...
```{r}
library(rpart)
tree <- rpart::rpart(depressed ~ E3 + E4 + E2 + V11 + E8 + country_agg, data = train_downsamp)
```
```{r}
