...
```{r}
set.seed(1234)
CTIS_micro$worried <- ifelse(CTIS_micro$D1 %in% levels(CTIS_micro$D1)[3:5], TRUE, FALSE )
...
```{r}
training.samples <- CTIS_micro$worried %>% 
  createDataPartition(p = 0.8, list = FALSE)
train.data  <- CTIS_micro[training.samples, ]
test.data <- CTIS_micro[-training.samples, ]

# Dumy code categorical predictor variables
x <- model.matrix(worried~., train.data)[,-1]
# Convert the outcome (class) to a numerical variable
y <- ifelse(train.data$worried == TRUE, 1, 0)
